...
def demonstrate_admin_vs_project_permissions():
    ...
    admin_gemini_client = GeminiCLIClient(
        working_directory="/tmp", timeout=30, is_admin_agent=True
    )

    print(
        f"   - Admin Gemini Client: is_admin_agent = {admin_gemini_client.is_admin_agent}"
    )
    ...
    project_gemini_client = GeminiCLIClient(
        working_directory="/tmp", timeout=30, is_admin_agent=False
    )

    print(
        f"   - Project Gemini Client: is_admin_agent = {project_gemini_client.is_admin_agent}"
    )
    ...
    example_tools = {
        ...
        "PythonDocumenter_Agent": ["Read", "Write", "Grep", "Glob", "Bash"],
        "KotlinEntityCreator_Agent": ["Read", "Write", "Grep", "Glob"],
        "TestRestrictedAgent_Claude": ["Read"],
    }
    ...
    print(
        "   ⚠️ Gemini: Project agents currently have full access (temporary limitation)"
    )
                    ...
                    with open(agent_yaml) as f:
                        ...
                        for line in lines:
                            if line.strip().startswith(
                                ("ai_provider:", "available_tools:")
                            ):
                                print(f"   {line.strip()}")
                            elif line.strip().startswith(
                                "- "
                            ) and "available_tools" in "".join(
                                lines[max(0, lines.index(line) - 3) : lines.index(line)]
                            ):
                                print(f"   {line.strip()}")
...
if __name__ == "__main__":
    ...
    print(f"\n" + "=" * 50)
    print(
        "Demo completed. Run tests with: python -m pytest tests/test_tool_permissions.py -v"
    )
