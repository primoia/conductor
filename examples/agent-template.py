...
class ConductorAgentBase(ABC):
    ...
    def _call_deepseek(self, system_message: str, user_message: str, max_tokens: int = 20) -> Dict:
...
class GradleVersionAgent(ConductorAgentBase):
    ...
    def _extract_version_with_deepseek(self, content: str, file_path: str) -> Dict:
        ...
        for line in content.split('\n')[:20]:  # Primeiras 20 linhas
            if any(keyword in line.lower() for keyword in ['gradle', 'version', 'distribution']):
...
Respond with only PASS or FAIL."""
        ...
        deepseek_result = self._call_deepseek(system_message, user_message, max_tokens=5)
