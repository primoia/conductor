...
def test_ollama_api_detailed():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"   Load Duration: {result.get('load_duration', 0) / 1_000_000:.0f}ms") 
            print(f"   Prompt Eval: {result.get('prompt_eval_duration', 0) / 1_000_000:.0f}ms")
            print(f"   Eval Duration: {result.get('eval_duration', 0) / 1_000_000:.0f}ms")
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"   🔤 Tokens: {result.get('prompt_eval_count', 0)}+{result.get('eval_count', 0)}")
...
def analyze_response_patterns():
    ...
    for i, example in enumerate(real_responses, 1):
        ...
        print(f"   ✅ Success: {'Yes' if example['extracted'] in ['PASS', 'FAIL', 'VALID', 'INVALID', 'OK', 'ISSUES'] or example['extracted'].replace('.', '').replace('-', '').isdigit() else 'No'}")
        print()
