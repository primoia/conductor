...
"""
    ...
    # Add recommendations section
    content += generate_recommendations_section(complexity_results, security_results, dependency_results)
...
"""
    ...
    top_issues = sorted(complexity_results, key=lambda x: x['complexity'], reverse=True)[:10]
...
"""
    ...
    for severity in ['critical', 'high', 'medium', 'low']:
        if severity in by_severity:
            ...
            content += f"#### {severity_icon} {severity.title()} ({len(issues)} issues)\n\n"
...
"""
    ...
    for severity in ['critical', 'high', 'medium', 'low']:
        if severity in by_severity:
            ...
            if len(issues) > 5:
                content += f"  - *... and {len(issues) - 5} more {severity} vulnerabilities*\n"
            content += "\n"
    
    return content


def generate_recommendations_section(complexity_results: List[Dict], security_results: List[Dict], dependency_results: List[Dict]) -> str:
...
"""
    ...
    if critical_security or critical_deps or critical_complexity:
        if critical_security:
            content += f"- Fix {len(critical_security)} critical security issues immediately\n"
        if critical_deps:
            content += f"- Update {len(critical_deps)} critical dependency vulnerabilities\n"
