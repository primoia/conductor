...
from core.state_repository import FileStateRepository, MongoStateRepository
...
class AdminAgent:
    ...
    def __init__(self, agent_id: str = None, ai_provider: str = None, logger = None, timeout: int = 90, state_provider: str = 'file'):
        """
        ...
        # Instanciar repositório de estado baseado no argumento
        if state_provider == 'mongo':
            state_repo = MongoStateRepository()
        else:
            state_repo = FileStateRepository()
        self.genesis = GenesisAgent(state_repository=state_repo)
...
Examples:
    ...
    parser.add_argument('--state-provider', type=str, default='file', choices=['file', 'mongo'],
                        help='State persistence provider: file (default) or mongo')
