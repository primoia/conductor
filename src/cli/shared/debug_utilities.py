...
class DebugUtilities:
    ...
    def save_debug_input(
        self, message: str, debug_dir: str = "/tmp/admin_debug"
    ) -> str:
        """
    ...
    def generate_simulation_response(self, message: str) -> str:
        ...
        if hasattr(self.cli_instance.agent_logic.llm_client, "conversation_history"):
            ...
            try:
                ...
                self.logger.warning(
                    f"Failed to save state in simulation mode: {save_error}"
                )

        agent_name = (
            self.cli_instance.agent_logic.get_current_agent() or "Unknown_Agent"
        )
...
"""
    ...
    def export_debug_report(self, output_path: str = None) -> str:
        ...
        with open(output_path, "w", encoding="utf-8") as f:
            ...
            f.write(f"- Embodied: {self.cli_instance.embodied}\n")
            f.write(
                f"- Working Directory: {self.cli_instance.agent_logic.working_directory}\n"
            )
            ...
            if hasattr(
                ...
                for i, msg in enumerate(history, 1):
                    ...
                    f.write(
                        f"Response: {msg.get('response', 'N/A')[:1000]}{'...' if len(msg.get('response', '')) > 1000 else ''}\n"
                    )
