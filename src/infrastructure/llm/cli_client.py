...
class BaseCLIClient(LLMClient):
    ...
    def __init__(
        ...
        self.genesis_agent = (
            None  # Reference to parent agent for access to tools and config
        )
        self.is_admin_agent = is_admin_agent  # Flag to identify admin agents
        logger.debug(
            f"BaseCLIClient initialized with working directory: {self.working_directory}, admin: {is_admin_agent}"
        )
    ...
    def invoke(self, prompt: str) -> str:
        ...
        try:
            ...
            if hasattr(self, "genesis_agent") and hasattr(
                self.genesis_agent, "get_available_tools"
            ):
            ...
            raise LLMClientError(
                f"Claude CLI timed out after {self.timeout} seconds. Complex operations may need more time."
            )
...
class GeminiCLIClient(BaseCLIClient):
    ...
    def invoke(self, prompt: str) -> str:
        ...
        try:
            ...
            MAX_PROMPT_LENGTH = (
                50000  # Reasonable limit to prevent "Argument list too long"
            )
            if len(prompt) > MAX_PROMPT_LENGTH:
                logger.warning(
                    f"Prompt too long ({len(prompt)} chars), truncating to prevent system errors"
                )
                prompt = (
            ...
            if self.is_admin_agent:
                ...
                logger.warning(
                    "Project agent: using yolo mode - tool restrictions not implemented in Gemini CLI yet"
                )
            ...
            logger.error(
                f"Gemini CLI failed with exit code {e.returncode}: {error_message}"
            )
            raise LLMClientError(f"Gemini CLI failed: {error_message}")
        except FileNotFoundError:
            logger.error(
                f"Gemini command not found: '{self.gemini_command}'. Make sure it's installed and in the PATH."
            )
