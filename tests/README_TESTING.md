...
### ✅ Testes CI-Safe (GitHub Actions) - 86 testes
```bash
...
| **CI Safe (GitHub Actions)** | `poetry run pytest -m "not e2e and not integration and not manual and not mongo"` | 86 CI-safe | ~3s ⚡ |
| **CI Helper Script** | `python run_ci_tests.py` | 86 CI-safe | ~3s ⚡ |
...
- **Python:** 3.11 e 3.12
- **Testes:** 86 CI-safe (sem Claude/Gemini/MongoDB/config.yaml)
