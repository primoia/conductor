...
def test_protection_mechanisms():
    ...
    assert (
        "❌ Chat failed" in result
    ), "AgentLogic deve retornar erro em vez de relançar exception"
    ...
    safety_check = repl._check_non_rate_limits()
...
def test_multiline_detection():
    ...
    for line, should_be_multiline in test_cases:
        ...
        print(
            f"   '{line}' → Multi-linha: {result} {'✅' if result == should_be_multiline else '❌'}"
        )
...
def test_rate_limiting_with_other_limits():
    ...
    repl.last_interaction_time = time.time()
    safety_check = repl._check_rate_limiting()
