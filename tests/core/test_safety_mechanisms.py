...
""")
    ...
    try:
        ...
        print(f"Truncation indicator present: {'[Mostrando últimas' in formatted_history}")
        
        # Test that it's within reasonable bounds
        assert len(formatted_history) < 10000, "History still too long after truncation"
        assert "[Mostrando últimas 5 de 20 interações]" in formatted_history, "Missing truncation indicator"
        ...
        with unittest.mock.patch('subprocess.run') as mock_run:
            ...
            try:
                ...
                print(f"Truncation marker present: {'[PROMPT TRUNCADO' in actual_prompt}")
                
                assert len(actual_prompt) <= 50000 + 100, "Prompt not properly truncated"  # +100 for truncation message
