...
class TestFullAgentLifecycle(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.test_agent_path = (
            cls.base_path / "projects" / "_common" / "agents" / cls.test_agent_id
        )
    ...
    def test_full_agent_lifecycle_e2e(self):
        ...
        create_command = [
            ...
            "AgentCreator_Agent",
            "--destination-path",
            str(self.test_agent_path.parent),
            "--input",
            f"Create a simple test agent named {self.test_agent_id} for E2E testing. This agent should be able to respond to basic greetings.",
        ]
        ...
        self.assertTrue(
            agent_yaml_path.exists(), f"agent.yaml not found: {agent_yaml_path}"
        )
        self.assertTrue(
            persona_md_path.exists(), f"persona.md not found: {persona_md_path}"
        )
        ...
        if agent_config is None:
            ...
            with open(agent_yaml_path, "r", encoding="utf-8") as f:
                ...
                self.assertTrue(len(agent_yaml_content) > 0, "agent.yaml is empty")
        else:
            self.assertIn("name", agent_config, "agent.yaml missing 'name' field")

        # Validate persona.md is not empty
        with open(persona_md_path, "r", encoding="utf-8") as f:
            persona_content = f.read().strip()
            self.assertTrue(len(persona_content) > 0, "persona.md is empty")
        ...
        self.assertTrue(
            len(execute_result.stdout) > 0, "No output received from agent execution"
        )
        ...
        self.assertTrue(
            len(conversation_history) > 0, "No conversation history recorded"
        )

        # Check if the last interaction contains our input
        last_interaction = conversation_history[-1]
        self.assertIn(
            "prompt", last_interaction, "Last interaction missing 'prompt' field"
        )
        self.assertIn(
            "response", last_interaction, "Last interaction missing 'response' field"
        )
